...
function resolveCommand(root, command) {
  if (process.platform === "win32") {
    ...
    const binExe = join(root, "node_modules", ".bin", `${command}.exe`);
...
function main(argv) {
  ...
  const resolvedCmd = resolveCommand(root, command);
  const targetCmd =
    process.platform === "win32" && resolvedCmd.includes(" ")
      ? `"${resolvedCmd}"`
      : resolvedCmd;
  const child = spawn(targetCmd, args, { stdio: "inherit", env, shell: process.platform === "win32" });
